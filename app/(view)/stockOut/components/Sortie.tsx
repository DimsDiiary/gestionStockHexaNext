...
export type SortieFormData = z.infer<typeof sortieSchema>;

interface SortiesProps {
  onClose: () => void;
  onSortieAdded: () => void;
  sortieToEdit?: SortieFormData & { id: string };
}
...
export default function Sorties({ onClose, onSortieAdded, sortieToEdit }: SortiesProps) {
  ...
  useEffect(() => {
    if (sortieToEdit) {
      Object.entries(sortieToEdit).forEach(([key, value]) => {
        setValue(key as keyof SortieFormData, value);
      });
      setDestination(sortieToEdit.chantierId ? 'chantier' : 'magasin');
      setSourceType(sortieToEdit.source ? 'quincaillerie' : 'magasin');
    }
  }, [sortieToEdit, setValue]);
  ...
  const onSubmit = async (data: SortieFormData) => {
    try {
      ...
      const url = sortieToEdit ? `/api/sortie/${sortieToEdit.id}` : '/api/sortie';
      const method = sortieToEdit ? 'PUT' : 'POST';
      ...
      if (response.ok) { 
        ...
        console.log(sortieToEdit ? 'Entrée modifiée avec succès:' : 'Entrée ajoutée avec succès:', result);
        onSortieAdded();
        onClose();
        reset();
      } else {
        const errorData = await response.json();
        console.error(sortieToEdit ? 'Erreur lors de la modification de l\'entrée:' : 'Erreur lors de l\'ajout de l\'entrée:', errorData);
      }
    } catch (error) {
