...
// Add this function at the top of your component or outside of it
...
export default function ProjetPage() {
    ...
    const fetchProjets = async () => {
        try {
            ...
            // Transform the data to remove underscores from property names
            const transformedData = data.map((projet: {
                _description: string;
                _budget: number;
                _date_fin: string; _id: string; _nom_projet: string; _os: string; _date_debut: string; 
}) => ({
                id: projet._id,
                nom_projet: projet._nom_projet,
                os: projet._os,
                date_debut: projet._date_debut,
                date_fin: projet._date_fin,
                budget: projet._budget,
                description: projet._description
            }));
            setProjets(transformedData);
        } catch (error) {
...
<TableBody>
    {projets.map((projet) => (
        <TableRow key={projet.id} className="hover:bg-gray-50">
            ...
            <TableCell>{formatDate(projet.date_fin)}</TableCell>
            <TableCell>
                {projet.budget != null 
                    ? Number(projet.budget).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) 
                    : ''}
            </TableCell>
            ...
            <TableCell className="text-right">
                ...
                <Button variant="ghost" size="sm" onClick={() => projet.id && handleDelete(projet.id)}>
